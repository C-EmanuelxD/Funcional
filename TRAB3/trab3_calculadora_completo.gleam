...
pub type Erros {
  ...
  ListaVazia
  //Erro de caso a disposição de operadores esteja de forma inválida
  OperadoresInvalidos
...
//Recebe uma expressão matematica e devolve o resultado
//de sua avalição completa.
//é verificadas as disposições dos parenteses, caso esteja errado retorna erro
//é verificado se a string está vazia, caso esteja retorna erro de entrada
//é verificado se a expressão está escrita de forma correta, caso possua
//valores não numericos ou operadores inválidos, retorna erro.
pub fn main_calculadora(entrada: String) -> Result(Int, Erros) {
  use _ <- result.try(verifica_espaco(entrada))
...
pub fn main_calculadora_examples() {
  ...
  check.eq(main_calculadora("( - 8 / 2 ) * 3 + ( - 4 ) - 6"), Ok(-22))
  check.eq(main_calculadora("4+7+8/"), Error(OperadoresInvalidos))
  check.eq(main_calculadora("4+a+8"), Error(SimboloInvalido))
  check.eq(main_calculadora("((4+4)+8"), Error(ParentesesInvalidos))
  check.eq(main_calculadora("4 4 + 55"), Error(EscritaInvalida))
  check.eq(main_calculadora("(-8/2)*3 3+(-4)-6"), Error(EscritaInvalida))
}

//Faz a verificação de espaçoes entre numeros dentro de uma string dada
//caso os numeros estejam formatados de forma errada ex.: 4 4 + 5
//Retorna erro de Escrita Invalida
pub fn verifica_espaco(lst: String) -> Result(Nil, Erros) {
  let list = string.split(lst, "")
  use _ <- result.try(list.fold(
    list,
    Ok([]),
    adiciona_e_verifica_espacamentos,
  ))

  Ok(Nil)
}

pub fn adiciona_e_verifica_espacamentos(
  ...
  case acc {
    ...
    [_, ..] ->
      case verifica_num(elem) {
        True -> checa_entre_strings(acc, elem)
        False -> Ok(list.append([elem], acc))
      }
  }
}

pub fn checa_entre_strings(acc: List(String), elem: String) -> Result(List(String), Erros){
  let lst_ate_num = list.take_while(acc, fn(a) { verifica_num(a) == False })
  case
    list.contains(lst_ate_num, "+")
    || list.contains(lst_ate_num, "*")
    || list.contains(lst_ate_num, "/")
    || list.contains(lst_ate_num, "-")
    || list.contains(lst_ate_num, "(")
    || list.contains(lst_ate_num, ")"),
    so_numero(lst_ate_num)
  {
    False, False -> Error(EscritaInvalida)
    False, True -> Ok(list.append([elem], acc))
    True, False -> Ok(list.append([elem], acc))
    _, _ -> Ok(list.append([elem], acc))
  }
}

//verifica se uma string possui apenas numeros.
pub fn so_numero(lst: List(String)) -> Bool {
  list.map(lst, verifica_num)
...
pub fn normaliza_lista_examples() {
  ...
  check.eq(normaliza_lista("-2+3"), Ok([Numero(-2), Operador(Soma), Numero(3)]))
  check.eq(
